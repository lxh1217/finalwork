...
# 尝试导入模型
try:
    from models import DiffKGReChorus
    print("成功导入 DiffKGReChorus")
...
def main():
    ...
    # 只在 load > 0 且不是第一次训练时加载模型
	if args.load > 0 and os.path.exists(args.model_path):
		model.load_model()
		logging.info(f'Loaded model from {args.model_path}')
	elif args.load > 0 and not os.path.exists(args.model_path):
		logging.warning(f'Model file {args.model_path} not found. Starting from scratch.')
...
if __name__ == '__main__':
	...
	try:
		import importlib
		# 先尝试导入模型模块
		if init_args.model_name == 'DiffKGReChorus':
			from models.DiffKGReChorus import DiffKGReChorus
			model_name = DiffKGReChorus
		else:
			model_module = importlib.import_module(f'models.{init_args.model_name}')
			model_name = getattr(model_module, f'{init_args.model_name}{init_args.model_mode}')
		...
		try:
			...
			# 如果还是失败，尝试直接导入
			exec(f'from models.{init_args.model_name} import {init_args.model_name}{init_args.model_mode}')
			model_name = eval(f'{init_args.model_name}{init_args.model_mode}')
