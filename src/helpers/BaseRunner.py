...
class BaseRunner(object):
	...
	def evaluate_method(predictions: np.ndarray, topk: list, metrics: list) -> Dict[str, float]:
		"""
		正确的评估方法
		predictions: [n_samples, n_candidates]，第一列是ground-truth
		"""
		# 确保predictions有正确的形状
		if predictions.shape[1] < 2:
			print(f"错误: predictions形状不正确: {predictions.shape}")
			print("应该为 [n_samples, n_candidates]，其中n_candidates >= 2")
			return {f'{metric}@{k}': 0.0 for metric in metrics for k in topk}
		
		# 获取ground-truth的预测分数
		gt_scores = predictions[:, 0]
		
		# 对于每个样本，计算ground-truth在排序中的位置
		gt_rank = []
		for i in range(len(predictions)):
			# 对当前样本的所有预测分数排序（降序）
			sorted_indices = np.argsort(predictions[i])[::-1]
			# 找到ground-truth（索引0）在排序中的位置
			rank = np.where(sorted_indices == 0)[0][0] + 1
			gt_rank.append(rank)
		
		gt_rank = np.array(gt_rank)
		...
		for k in topk:
			...
			for metric in metrics:
				...
				if metric == 'HR':
					...
					# 计算NDCG：ground-truth排在第一位得分为1/log2(2)=1，第二位得分为1/log2(3)=0.63，以此类推
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
			...
			# 确保batch包含多个候选物品
			if 'item_id' in batch and batch['item_id'].shape[1] < 2:
				print(f"警告: 批处理中候选物品数量不足: {batch['item_id'].shape}")
				continue
			
			# 获取预测
			if hasattr(dataset.model, 'inference'):
				prediction = dataset.model.inference(batch)['prediction']
			else:
				prediction = dataset.model(batch)['prediction']
			
			all_predictions.append(prediction.cpu().data.numpy())
		...
		print(f"候选物品数量: {predictions.shape[1]}")
